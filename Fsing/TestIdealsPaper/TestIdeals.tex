...
\address{Department of Pure Mathematics, University of Sheffield, Hicks Building, Sheffield S37RH, United Kingdom}
\email{M.Katzman@sheffield.ac.uk}

\author[]{Sara Malec}
\address{Hood College, 401 Rosemont Ave. Frederick, MD 21701}
...
\[
(I^{[p^e]} : I) \cap (\Omega^{[p^e]} : \Omega)\over I^{[p^e]}
\]
where $I \subseteq \Omega \subseteq S$ is an ideal whose image in $R$ is isomorphic to the canonical module of $R$, see \cite{KatzmanParameterTestIdealOfCMRings} for details.
...
\todo[inline]{
   Make up a better example, with non-principal ideal.
}
\medskip
{\small
\begin{MyVerbatim}
...
i4 : f = inducedMap( coker gens I, coker gens frobenius I );
i5 : resf = res f
          1             1
o5 = 0 : R  <--------- R  : 0
               | 1 |
...
The method \texttt{ascendModule} can be used to calculate  $B$ (see detailed description of similar method \texttt{ascendIdeal} in Section \ref{Section: Test Ideals}).

\medskip
{\small
\begin{MyVerbatim}
i13 : V = ascendModule( 1, A, U )
o13 = | 0   a  bc  |
      | c+d d2 a2d |
\end{MyVerbatim}
}\medskip
...
i2 : isFInjective R
o2 = false
i3 : isFInjective( R, IsLocal => true )
o3 = true
\end{MyVerbatim}
}\medskip
...
i6 : isFRegular( 5/6, f )
o6 = false
i7 : isFRegular( 5/6, f, IsLocal => true )
o7 = true
\end{MyVerbatim}
}\medskip
...
the tensor of this map with any $R$-module remains injective.  If $R^{1/p}$ is a finite $R$-module, this is equivalent to requiring that $R \to R^{1/p}$ splits as a map of $R$-modules.
\end{definition}
