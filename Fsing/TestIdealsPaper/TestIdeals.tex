...
This paper describes methods for computing objects and numerical invariants in prime characteristic commutative algebra, implemented in the {\tt TestIdeals.m2} package for the computer algebra system Macaulay2.
A ring $R$ of prime characteristic $p>0$ comes equipped with the Frobenius endomorphism
\[ F: R \to R \ \text{ given by } F(x) =  x^p \]
which is the basis for many constructions and definitions.  Notably, Frobenius can be used to detect if a ring is regular \cite{KunzCharacterizationsOfRegularLocalRings} and more generally can be used to evaluate the severity of singularities.
...
(cf. the survey \cite{SchwedeTuckerTestIdealSurvey}).  These methods are used by a wide variety of commutative algebraists and algebraic geometers.
%That said, it remains a fundamental tool in the expanding area of prime characteristic commutative algebra.

The {\tt TestIdeals.m2} package was started during a \emph{Macaulay2} development workshop in 2012, hosted by Wake Forest University (at the time, the package was called {\tt PosChar.m2}.  The goal of this package was to provide a unified and efficient set of tools to study singularities in characteristic $p > 0$, and in particular collect and implement algorithms that had been described in several papers.  The package {\tt PosChar.m2} was split into two packages, {\tt TestIdeals.m2} and {\tt FThresholds.m2}.
%Its main goals were to implement basic functionality in prime characteristic, to collect useful algorithms that had already been implemented by various authors, and to implement other useful algorithms.
Since then, much more functionality has been added by many contributors, including in several more \emph{Macaulay2} development workshops.\footnote{Development workshops hosted by the University of California, Berkeley (2014, 2017), Boise State University (2015), and the University of Utah (2016).}
...
The authors thank those who have made contributions to the {\tt TestIdeals.m2} package, which includes
...
This paper, and the finishing touches to the {\tt TestIdeals.m2} package, were made at the University of Utah in 2018, and the authors visiting thank the Department of Mathematics for its hospitality.
...
Let $S$ denote a commutative ring of prime characteristic $p$.

\begin{definition}
Given an ideal $I\subseteq S$ and an integer $e\geq 0$, we define the \emph{$e$-th Frobenius power of $I$}, denoted $I^{[p^e]}$, to be the ideal
...
\begin{definition}
Given an ideal $I\subseteq S$ and an integer $e\geq I$, we define the \emph{$e$-th Frobenius root of $I$}, denoted $I^{[1/p^{e}]}$, to be the smallest ideal $J$ such that $I\subseteq J^{[p^e]}$, if such an ideal exists.
\end{definition}

Frobenius roots exist in polynomial and power series rings
(cf.~\cite[\S 2]{BlickleMustataSmithDiscretenessAndRationalityOfFThresholds} and \cite[\S 5]{KatzmanParameterTestIdealOfCMRings}).  They also exist in $F$-finite regular rings.  Below is an example where they are computed, and we describe the idea behind their computation below.%In all such rings.
...
We can also describe Frobenius roots as follows.  In a (sufficiently local) regular ring, we have an identification $R \cong \omega_R$.  On the other hand the Grothendieck dual of the $e$-iterated Frobenius map\footnote{In a reduced ring $R$, it is often convenient to express the Frobenius endomorphism $R \to R$ as the inclusion $R \hookrightarrow R^{1/p}$. The point is the source and target of the latter map are distinguished.} $R \to R^{1/p^e}$ provides a map
\begin{equation}
...
T : R^{1/p^e} \to R.
\]
It is not difficult to prove that $T(I^{1/p^e}) = J$ (here $I^{1/p^e} \subseteq R^{1/p^e}$ is simply the ideal of $p^e$th roots of elements of $I$).  On the other hand, in the case that
\[
R = \bF_q[x_1, \ldots, x_d],
\]
then $R^{1/p^e}$ is a free $R$-module with basis
\[
\{ x_1^{\lambda_1/p^e} \cdots x_d^{\lambda_d/p^e} \}_{0 \leq \lambda_i \leq p^e-1}.
\]
...
\[
 = \sum f_{\bf \lambda} {\bf x}^{{\bf \lambda} / p^e}
\]
...
If $I = (f)$, then again $\omega_R \cong R = S/I$ and the action of $T_R$ can be computed on $S$.  In fact, if $\overline{J} \subseteq R \cong \omega_R$ corresponds to an ideal $J \subseteq S$, then
\[
T_R({\overline J}^{1/p^e}) = \overline{(f^{p^e-1} J)^{[1/p^e]}}.
\]
More generally, for non-hypersurfaces, the analog to $f$ is chosen carefully out of $I^{[p^e]} : I$.  See \cite{KatzmanParameterTestIdealOfCMRings} for details.  Here we compute two examples.

\begin{verbatim}
i2 :      S=ZZ/5[x,y,z];
i3 :      g=x^3+y^3+z^3;
i4 :      u=g^(5-1);
i5 :      frobeniusRoot(1,ideal(u))
o5 = ideal (z, y, x)
...
\rightarrow R^{1/p}$ using the {\tt PushForward} package \cite{}. 
...
The Frobenius action on top local cohomology (dual to $\omega_R$) is usually computed in a different (faster) way than the other cohomologies, and this is modified by the {\tt CanonicalStrategy} option.  The default option is {\tt CanonicalStrategy => Katzman} which instead relies on the fact that we already know how to compute the Frobenius action on the canonical module, as described in \autoref{subsec.DualToFrobeniusOnQuotientRings}.  
...
%The default value for the option {\tt AssumeCM} is {\tt false}.
%Of
...
When entering a reduced ring,  setting {\tt AssumedReduced => true} avoids
computing the bottom local cohomology, and when entering a normal ring, setting
...
All of these checks are done by actually computing the test ideal (as described in the next section).  
...
\subsection{$F$-purity}
\begin{definition}
A ring $R$ is called \emph{$F$-pure} if the inclusion $R \subset
R^{1/p^{e}}$ split.
\end{definition}
...
$E=E_{R_\mathfrak{m}}(R_{\mathfrak{m}}/\mathfrak{m})=H^n_{\mathfrak{m}} (R)$, and let $\Theta: E \rightarrow E$ be the $p^e$-linear map
...
We hope to achieve this during a future \emph{Macaulay2} workshop.
